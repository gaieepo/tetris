...
def run_game(weights, explore_change):
    ...
    while True:  # game loop
        if falling_piece is None:
            ...
            current_move, weights = gradient_descent(
                board, falling_piece, weights, explore_change
            )
        ...
        for event in pygame.event.get():  # event handling loop
            if event.type == keys.KEYUP:
                ...
                if (
                    event.key == keys.K_LEFT or event.key == keys.K_a
                ) and is_valid_position(board, falling_piece, adj_x=-1):
                    falling_piece['x'] -= 1
                    moving_left = True
                    moving_right = False
                    last_lateral_time = time.time()
                    ...
                    event.key == keys.K_RIGHT or event.key == keys.K_d
                ) and is_valid_position(board, falling_piece, adj_x=1):
                    falling_piece['x'] += 1
                    moving_right = True
                    moving_left = False
                ...
                # rotating the piece (if there is room to rotate)
                    ...
                    ) % len(PIECES[falling_piece['shape']])
                    if not is_valid_position(board, falling_piece):
                        falling_piece['rotation'] = (
                            falling_piece['rotation'] - 1
                        ) % len(PIECES[falling_piece['shape']])
        ...
        ) and time.time() - last_lateral_time > MOVESIDEWAYSFREQ:
            if moving_left and is_valid_position(
                board, falling_piece, adj_x=-1
            ):
                falling_piece['x'] -= 1
            elif moving_right and is_valid_position(
                board, falling_piece, adj_x=1
            ):
                falling_piece['x'] += 1
            last_lateral_time = time.time()
        ...
        if (
            moving_down
            and time.time() - last_move_down_time > MOVEDOWNFREQ
            and is_valid_position(board, falling_piece, adj_y=1)
        ):
            falling_piece['y'] += 1
            last_move_down_time = time.time()
            games_completed += 1
...
def show_text_screen(text):
    ...
    press_key_surf, press_key_rect = make_text_objs(
        'Please wait to continue.', BASICFONT, TEXTCOLOR
    )
...
def is_valid_position(board, piece, adj_x=0, adj_y=0):
    ...
    for x in range(TEMPLATEWIDTH):
        for y in range(TEMPLATEHEIGHT):
            ...
            if (
                is_above_board
                or PIECES[piece['shape']][piece['rotation']][y][x] == BLANK
            ):
                continue
...
def draw_status(score, level, best_move):
    ...
    move_surf = BASICFONT.render(
        'Current Move: %s' % best_move, True, TEXTCOLOR
    )
    ...
    for i in range(0, BOARDWIDTH):
        ...
        for j in range(0, BOARDHEIGHT):  # Scan from top to bottom
            if int(board[i][j]) > 0:
                occupied = (
                    1
                )  # If a block is found, set the 'Occupied' flag to 1
...
def get_expected_score(test_board, weights):
    ...
    D = weights[3]
    test_score = float(
        A * height_sum + B * diff_sum + C * max_height + D * holes
    )
    return test_score
...
def simulate_board(test_board, test_piece, move):
    ...
    for i in range(0, rot):
        test_piece['rotation'] = (test_piece['rotation'] + 1) % len(
            PIECES[test_piece['shape']]
        )
    ...
    height_sum, diff_sum, max_height, holes = get_parameters(test_board)
    one_step_reward = 5 * (test_lines_removed * test_lines_removed) - (
        height_sum - reference_height
    )
...
if __name__ == '__main__':
    ...
    while True:  # game loop
        ...
        print(
            "Game Number ", games_completed, " achieved a score of: ", newScore
        )
